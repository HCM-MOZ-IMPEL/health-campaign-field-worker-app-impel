...
class _CustomDigitScannerPageState
  ...
  RegExp manualCodeReg = RegExp(r'^[a-zA-Z0-9]{10}$');
      ...
      body: BlocBuilder<DigitScannerBloc, DigitScannerState>(
        builder: (context, state) {
          return _cameras.isNotEmpty
              ? !manualCode
                  ? _defaultScanner(context, theme, state)
                  : _manualScanner(theme)
              : const Center(
                            ...
                            children: [
                              Flexible(
                                child: Text(
                                  ...
                                  widget.isGS1code
                                      ? state.barCodes[index].elements.entries
                                          .last.value.data
                                          .toString()
  ...
  Widget _manualScanner(ThemeData theme) {
                ...
                footer: DigitElevatedButton(
                  onPressed: () async {
                    if (form.control(_manualCodeFormKey).value == null ||
                      ...
                      BuildContext _currentContext = context;
                      final bloc = context.read<DigitScannerBloc>();
                      String code = form.control(_manualCodeFormKey).value;
                      if (widget.isGS1code && manualCodeReg.hasMatch(code)) {
                        final String barcode = '$gs1Key$code';
                        try {
                          final parser = GS1BarcodeParser.defaultParser();
                          GS1Barcode? dataResult = parser.parse(barcode);
                          List<String?> barCodes = result
                              .map((e) => e.getAIsRawData[gs1Key])
                              .toList();
                          if (result.length >= widget.quantity) {
                            await DigitToast.show(
                              ...
                              .contains(dataResult.getAIsRawData[gs1Key])) {
  ...
  Future<void> storeCodeWrapper(String code) async {
    if (codes.length < widget.quantity) {
      ...
      await DigitToast.show(
        ...
        options: DigitToastOptions(
          localizations
              .translate(i18_local.deliverIntervention.bednetScanMoreThanCount),
          true,
          Theme.of(currentContext),
        ),
      );

...
class ProjectBeneficiarySearchModelMapper extends BaseMapper<ProjectBeneficiarySearchModel> {
  ...
  @override String stringify(ProjectBeneficiarySearchModel self) => 'ProjectBeneficiarySearchModel(boundaryCode: ${Mapper.asString(self.boundaryCode)}, isDeleted: ${Mapper.asString(self.isDeleted)}, auditDetails: ${Mapper.asString(self.auditDetails)}, additionalFields: ${Mapper.asString(self.additionalFields)}, id: ${Mapper.asString(self.id)}, projectId: ${Mapper.asString(self.projectId)}, beneficiaryId: ${Mapper.asString(self.beneficiaryId)}, tag: ${Mapper.asString(self.tag)}, beneficiaryClientReferenceId: ${Mapper.asString(self.beneficiaryClientReferenceId)}, beneficiaryRegistrationDateLte: ${Mapper.asString(self.beneficiaryRegistrationDateLte)}, beneficiaryRegistrationDateGte: ${Mapper.asString(self.beneficiaryRegistrationDateGte)}, limit: ${Mapper.asString(self.limit)}, offset: ${Mapper.asString(self.offset)}, clientReferenceId: ${Mapper.asString(self.clientReferenceId)}, tenantId: ${Mapper.asString(self.tenantId)}, dateOfRegistrationTime: ${Mapper.asString(self.dateOfRegistrationTime)})';
  @override int hash(ProjectBeneficiarySearchModel self) => Mapper.hash(self.boundaryCode) ^ Mapper.hash(self.isDeleted) ^ Mapper.hash(self.auditDetails) ^ Mapper.hash(self.additionalFields) ^ Mapper.hash(self.id) ^ Mapper.hash(self.projectId) ^ Mapper.hash(self.beneficiaryId) ^ Mapper.hash(self.tag) ^ Mapper.hash(self.beneficiaryClientReferenceId) ^ Mapper.hash(self.beneficiaryRegistrationDateLte) ^ Mapper.hash(self.beneficiaryRegistrationDateGte) ^ Mapper.hash(self.limit) ^ Mapper.hash(self.offset) ^ Mapper.hash(self.clientReferenceId) ^ Mapper.hash(self.tenantId) ^ Mapper.hash(self.dateOfRegistrationTime);
...
abstract class ProjectBeneficiarySearchModelCopyWith<$R> {
  ...
  $R call({String? id, String? projectId, String? beneficiaryId, String? tag, List<String>? beneficiaryClientReferenceId, DateTime? beneficiaryRegistrationDateLte, DateTime? beneficiaryRegistrationDateGte, int? limit, int? offset, List<String>? clientReferenceId, String? tenantId, int? dateOfRegistration, String? boundaryCode});
